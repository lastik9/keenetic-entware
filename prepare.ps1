...
function Ensure-UsbipFirewall {
  ...
  if (Get-NetFirewallRule -DisplayName $script:UsbipFwRuleName -ErrorAction SilentlyContinue) {
    ...
    Write-Host "    Разреши в нём входящий TCP 3240 и запусти prepare.ps1 снова." -ForegroundColor Cyan
    return
  }
  ...
  if ((Test-UsbipPort $hostIp) -eq $true) {
    ...
    Write-Host "    Разреши входящий TCP 3240 в нём и запусти prepare.ps1 снова." -ForegroundColor Cyan
...
function Detach-Usbipd {
  ...
  # Писалось оно под эпоху '--auto-attach', который оставлял висеть долгоживущий
  # usbipd.exe. Auto-attach больше не используется (см. комментарий у attach выше),
  # и единственный процесс с именем 'usbipd' в системе — САМА СЛУЖБА usbipd.
  # Проверено 17.07.2026 на Win10: PID службы (Win32_Service.ProcessId) совпадает
  # с Id из Get-Process usbipd. То есть обёртка на выходе расстреливала службу,
  ...
  if ($script:UsbipBusid) {
    ...
    try {
      ...
      # ~6,0 c (замер 17.07.2026 днём, ASMedia). unbind в этом окне честно печатает
