...
FSCK_FIX="${FSCK_FIX:-0}"      # 1 — при грязной ФС чинить e2fsck -fy (согласие дала обёртка); ПИШЕТ во флешку
...
# Чинит ФС на месте (e2fsck -fy), затем перепроверяет (-fn). ПИШЕТ во флешку-источник.
# Возвращает 0 и ставит FS_IS_CLEAN=1, если ФС стала чистой; 1 — если ошибки остались.
# ВНИМАНИЕ: вызывать только в условии (if/||) — под set -e голый вызов с return 1 убьёт скрипт.
run_fsck_fix() {
  local part="$1" fy_rc=0
  ...
  if "$E2FSCK" -fn "$part" >/dev/null 2>&1; then ok "ФС чистая."; FS_IS_CLEAN=1; return 0; fi
  return 1
}
...
fsck_precheck() {
  ...
  # Починка e2fsck -fy ПИШЕТ во флешку-источник, поэтому только с явного согласия —
  # молча под ASSUME_YES не правим (та же этика, что в интерактиве: сначала спросить).
  if [[ "$ASSUME_YES" == "1" ]]; then
    # Обёртка неинтерактивна — read невозможен. FSCK_FIX=1 = согласие уже получено
    # обёрткой (она спросила [Y/n] в PowerShell) и передано вниз. Без флага выходим
    # особым кодом 3, чтобы обёртка отличила «грязная ФС» от прочих сбоев и спросила.
    if [[ "$FSCK_FIX" == "1" ]]; then
      if run_fsck_fix "$part"; then return 0; fi
      die "ФС всё ещё с ошибками после e2fsck -fy — образ не снимаю (флешка образом не тронута)."
    fi
    err "ФС грязная, а прогон неинтерактивный (ASSUME_YES=1)."
    err "Разреши починку: перезапусти с FSCK_FIX=1 (обёртка предложит это сама), либо запусти скрипт в терминале для интерактивной починки."
    exit 3
  fi
  read -rp "Что делать? [f] починить (e2fsck -fy)  [c] продолжить как есть  [q] выход: " ans < "$TTY"
  case "$ans" in
    f|F)
      if run_fsck_fix "$part"; then :
