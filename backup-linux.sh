...
fsck_precheck() {
  ...
  # Неинтерактивный прогон (обёртка ставит ASSUME_YES=1) не может ответить на read.
  # Молча ФС не правим (e2fsck -fy на повреждённой ФС разрушителен) и битую не имажим —
  # падаем внятно. Чинить интерактивно: запусти скрипт напрямую в терминале.
  if [[ "$ASSUME_YES" == "1" ]]; then
    die "ФС грязная, а прогон неинтерактивный (ASSUME_YES=1). Размонтируй флешку чисто и повтори, либо запусти скрипт напрямую в терминале для интерактивной починки."
  fi
