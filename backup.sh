...
do_backup() {
  ...
  # В dry-run файла НЕТ — путь не выдумываем, иначе clone пойдёт дальше впустую.
  [[ "$DRY_RUN" == "1" ]] || LAST_BACKUP="$PWD/$OUT"
  ...
  echo
  if [[ "$DRY_RUN" == "1" ]]; then
    ok "Dry-run завершён — диск НЕ тронут, файл образа НЕ создан."
    info "Файл был бы: $PWD/$OUT"
  else
...
do_clone() {
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    echo
    warn "(dry-run) образ не создан — клонировать нечего. Проверка на этом закончена."
    exit 0
  fi
