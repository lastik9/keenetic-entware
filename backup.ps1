...
function Attach-Usbipd($disk) {
  ...
  for ($a = 1; $a -le 5; $a++) {
    ...
    if ($out -match 'error|failed|state') { Warn "Попытка $a не удалась, повтор через 3 c..."; Start-Sleep -Seconds 3 }
    else { $attached = $true; break }
  }
  if (-not $attached) { Start-Sleep -Seconds 2 }
