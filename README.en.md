...
3. **Add a share** with the "+ Add share" button, pointing to the currently mounted `OPKG` partition. Verify it's mounted in **Applications → Disks and printers** (status "Connected", filesystem `EXT4`). In the folder-selection dialog that opens, set the `admin` user's access right for that folder to **Read & Write** — without it SMB won't let `admin` in at all (and **Read-only** would let you connect but not save the edits the share exists for).
